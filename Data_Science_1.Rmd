...
bachelor2020_qqplots <- qqnorm(data_counties$bachelor_degree_numbers_2016_2020,
                         main = "QQ Plot of Bachelor degree number 2016 - 2020",
                         xlab = "Theoretical Quantiles",
                         ylab = "Sample Quantiles",
                         plot.it = TRUE,
                         datax = FALSE)
...
```{r distribution for associate}
p_associate <- ggplot(data_counties, 
...
p_associate_2 <- ggplot(data_counties, 
                 aes(x = associate_degree_numbers_2016_2020_No_Outliers)) +
  geom_histogram(bins = 40, fill = "#1f77b4", color = "white", alpha = 0.9) +
  scale_x_continuous(labels = comma) +
  labs(
    title = "Associate Degree Numbers (2016-2020, No Outliers)",
    x = "Number of Associate Degrees",
    y = "Count"
  ) +
  theme_minimal(base_size = 13) +
  theme(
...
```{r new qqplot bachelor 2020}
bachelor2020_qqplots_NoOut <- qqnorm(data_counties$bachelor_degree_numbers_2016_2020_No_Outliers,
...
associate2020_qqplots_NoOut <- qqnorm(data_counties$associate_degree_numbers_2016_2020_No_Outliers,
...
bachelor_ttest <- t.test(x=data_counties$bachelor_degree_percentage_2016_2019, conf.level=0.95)
...
ggplot(state_means_bachelor_No_Outliers, aes(x = reorder(state, bachelor_degree_numbers), y = bachelor_degree_numbers)) +
  geom_col(fill = "skyblue") +
  labs(title = "Number of bachelor degree from 2016-2020 by State without Outliers",
       x = "State",
       y = " Bachelor degree numbers") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 8))
...
ggplot(state_means_associate_No_Outliers, aes(x = reorder(state, associate_degree_numbers), y = associate_degree_numbers)) +
  geom_col(fill = "skyblue") +
  labs(title = "Number of Associate degree from 2016-2020 by State without outliers",
       x = "State",
       y = " Associate degree numbers") +
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 8))
...
data_counties <- data_counties %>%
  mutate(income_cat = case_when(
      ...
      bachelor_degree_percentage_2016_2019 < quantile(bachelor_degree_percentage_2016_2019, 0.33, na.rm = TRUE) ~ "Low",
      bachelor_degree_percentage_2016_2019 < quantile(bachelor_degree_percentage_2016_2019, 0.67, na.rm = TRUE) ~ "Medium",
      TRUE ~ "High"),
...
ggplot(data_counties, aes(x = bachelor_degree_percentage_2016_2019)) +
  geom_density(fill = "green", alpha = 0.6, color = "darkgreen") +
  geom_vline(aes(xintercept = mean(bachelor_degree_percentage_2016_2019, na.rm = T)), 
             color = "red", linetype = "dashed", size = 1) +
  theme_minimal() +
  labs(
