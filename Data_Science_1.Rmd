...
# Motivation for our data science project
...
```{r variables table}
descriptions <- c(
  county_FIPS      = "5-digit county FIPS code (string, zero-padded)",
...
- Standardization: Key identifiers - county_FIPS appear with 4 digits instead of 5, which can cause merge errors with other sources.
...
## Per Capita Personal Income at the county level visualization
...
data_counties <- data_counties %>%
  mutate(income_cat = case_when(
      per_capita_personal_income_2020 < quantile(per_capita_personal_income_2020, 0.25, na.rm = TRUE) ~ "Low",
      per_capita_personal_income_2020 < quantile(per_capita_personal_income_2020, 0.75, na.rm = TRUE) ~ "Medium",
      TRUE ~ "High"),
      bachelor_cat = case_when(
      bachelor_degree_percentage_2015_2019 < quantile(bachelor_degree_percentage_2015_2019, 0.25, na.rm = TRUE) ~ "Low",
      bachelor_degree_percentage_2015_2019 < quantile(bachelor_degree_percentage_2015_2019, 0.75, na.rm = TRUE) ~ "Medium",
      TRUE ~ "High"),
      associate_cat = case_when(
      associate_degree_percentage_2016_2020 < quantile(associate_degree_percentage_2016_2020, 0.25, na.rm = TRUE) ~ "Low",
      associate_degree_percentage_2016_2020 < quantile(associate_degree_percentage_2016_2020, 0.75, na.rm = TRUE) ~ "Medium",
      TRUE ~ "High"))
