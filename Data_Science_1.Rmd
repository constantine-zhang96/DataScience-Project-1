...
```{r qq plot income 2019}
income2019_qqplots <- qqnorm(data_counties$per_capita_personal_income_2019,
                         main = "QQ Plot of Personal Income Per Capita in 2019",
                         xlab = "Theoretical Quantiles",
                         ylab = "Sample Quantiles",
                         plot.it = TRUE,
                         datax = FALSE)
qqline(data_counties$per_capita_personal_income_2019, 
       col = 2,
       lwd = 2)
```
...
```{r qqlot income 2021}
income2021_qqplots <- qqnorm(data_counties$per_capita_personal_income_2021,
                         main = "QQ Plot of Personal Income Per Capita in 2021",
                         xlab = "Theoretical Quantiles",
                         ylab = "Sample Quantiles",
                         plot.it = TRUE,
                         datax = FALSE)
qqline(data_counties$per_capita_personal_income_2021, 
       col = 2,
       lwd = 2)
...
income_no_outlier <- head(df_long_no_out, 5)
names(income_no_outlier) <- c("County FIPS","State","County", "Year",
                "Per-Capita Income")

kable(income_no_outlier, caption = "Table 1. First 5 rows of Per-capita counties data (Removed Ouliers)",
      format = "html", align = "c") |>
  kableExtra::kable_styling(bootstrap_options = c("striped","hover","condensed"),
                            full_width = FALSE, position = "center") |>
  kableExtra::scroll_box(width = "100%", height = "300px")
...
```{r plot the new qqplot}
p_hist <- ggplot(df_long_no_out, aes(x = income, fill = factor(year))) +
  geom_histogram(bins = 30, color = "white", alpha = 0.95) +
  facet_wrap(~ year, scales = "free_y") +
  scale_fill_manual(values = c("2019" = "#1f77b4",
                               "2020" = "#2ca02c",
                               "2021" = "#ff7f0e")) +
  scale_x_continuous(labels = scales::comma) +
  labs(title = "Per-capita Personal Income (IQR outliers removed)",
       x = "Income", y = "Count", fill = "Year") +
  theme_minimal() +
  theme(legend.position = "none")
